...
\item \emph{Berlin TEEG-ALT-v2 dominance over CSA}: machine
  A $2.34\times$ (Table~1 single-run), machine A $4.13\times$ (Table~S6
  $N=3$ bootstrap; tighter measurement), machine B (server pkg v2)
  $2.02\times$ ($1.88$ vs $3.79$ ms; 498 OD subset). Cross-machine
  dominance robust; the bootstrap estimate is the most reliable
  because it averages JIT-warmup variance across 3 independent runs.
\item \emph{NYC TEEG-ALT-v2 dominance over CSA}: machine A
  $2.30\times$ (Table~1 single-run); machine A $4.98\times$ (Table~S6
  $N=3$ bootstrap). NYC was not benched on machine B (server v2 KEY 3
  ran Berlin only).
...
\item \emph{Server pkg v2 partial-run note}: 3 of 7 KEYs completed
  ...
  (rc=$-9$) at the Full London 15-min walk-cap. Server pkg v3 with
  10-min walk-cap default + split-pass Cycle 10 +
  $\{3,5,7,10,12\,\text{min}\}$ walk-cap sweep is in flight; see
  \texttt{Experiments/results/cycle\_9\_paper/}\allowbreak\texttt{server\_v2\_results/SUMMARY.md}.
\end{itemize}
