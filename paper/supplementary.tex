...
\paragraph{Cross-city multi-pass-CSA blind-spot rates.}
To check whether the 6.17\,\% rate is a Full-London artefact, we ran
the same multi-pass oracle on 500-OD subsets of Berlin and NYC.
Berlin: \textbf{3/500 = 0.6\,\%} (median $\Delta\!=\!-2$\,min, range
$-1$ to $-16$\,min). NYC: \textbf{6/500 = 1.2\,\%} (median
$\Delta\!=\!-2.5$\,min, range $-1$ to $-9$\,min). The phenomenon is
general but rate varies sharply with transit-topology density:
London (12.2 walks/stop, dense overlapping tube+bus) has $5\text{--}10\times$
the blind-spot rate of Berlin (9.0 walks/stop) and NYC (16.5
walks/stop). Per-query results in
\texttt{Experiments/results/cycle\_9\_paper/multipass\_berlin\_nyc/}\allowbreak\texttt{summary.json}.
...
\paragraph{Cross-machine summary (post server-v3 validation 2026-06-14).}
\begin{itemize}
\item \emph{Berlin TEEG-ALT-v2 dominance over CSA}: machine
  A $2.34\times$ (Table~1 single-run), machine A $4.43\times$ (Table~S6
  $N=5$ bootstrap; tighter measurement), machine B server pkg v2
  $2.02\times$ ($1.88$ vs $3.79$ ms; 498 OD), machine B server pkg
  v3 $2.12\times$ ($1.63$ vs $3.46$ ms). Cross-machine dominance
  robust; bootstrap is the most reliable.
\item \emph{NYC TEEG-ALT-v2 dominance over CSA}: machine A
  $2.30\times$ (Table~1 single-run); machine A $4.99\times$ (Table~S6
  $N=5$ bootstrap). NYC was not benched on machine B (server v3 KEY 3
  ran Berlin only).
\item \emph{Cycle 10 dynamic update speedup}: machine A single-edit
  $12.5$ ms $/ 285$ s rebuild $= 45{,}755\times$; machine B server v3
  single-edit $0.47$ ms $/ 280$ s rebuild $= \mathbf{596{,}346\times}$
  (13$\times$ better than machine A; Numba JIT performance on server's
  8 vCPU outperforms dev box's i7 for this workload).
\item \emph{Lazy-CSA-resort speedup}: machine A median $15.3\times$
  (12 Cycle 10 cells); machine B server v3 median
  $\mathbf{35.8\times}$ (same 12 cells), range $13.5\text{--}50.0\times$.
  Server-v3 medians used in Table~\ref{tab:lazy-csa} of main paper.
\item \emph{Cycle 10 Berlin oracle correctness}: machine B v2+v3
  both reproduce dev-box --- 14/3600 CSA-better at N=1000 (0.39\%),
  36/3600 TEEG-better (1.00\%); 11 of 12 cells at $\geq 99\%$ match,
  single outlier add\_N1000 at 97.3\%.
\item \emph{17-residual multi-pass oracle}: machine B v2+v3 both
  reproduce bit-for-bit --- multi-pass CSA matches TEEG-ALT on 16/17
  residuals; strictly improves over single-pass on 17/17.
\item \emph{Full London TEEG-ALT-v2 vs CSA on machine B}: 3-rep
  bootstrap v2 gives CSA $8.34$ ms CI95\% [$7.82, 8.69$] vs
  TEEG-ALT-v2 $12.69$ ms [$11.57, 13.80$] (1.52$\times$ CSA-faster);
  v3 single-run gives CSA $7.52$ vs TEEG-ALT-v2 $10.16$ ms
  (1.35$\times$ CSA-faster). Consistent direction; machine A reports
  the opposite ratio ($1.10\times$ TEEG-faster). We treat the Full
  London static-latency comparison as within-hardware-class artefact.
\item \emph{Walk-cap sweep robust at server pkg v3}: 5-point sweep
  $\{3,5,7,10,12\}$ min completed in 27 min on 40\,GB Xeon (server v2
  $\{10,20\}$ OOM at 20-min cap). Numbers in supp Table~S7.
\item \emph{Server pkg v3 full success}: all 7/7 KEYs returned rc=0
  in 119 min wall total (v2 4/7 OOM; v3 fixes: 10-min walk-cap
  default + split-pass Cycle 10 + smaller 5-point sweep).
\end{itemize}

\begin{table}[H]
\centering
\caption{Server pkg v3 walk-cap sweep, Full London, 500 ODs per cap
on machine B (Xeon 8260L). Median ms/q per method.}
\label{tab:walkcap-sweep-v3}
\small
\begin{tabular}{rrrrr}
\toprule
max\_walk\_min & TEEG-ALT-v2 & CSA & RAPTOR & HL \\
\midrule
 3 &  8.20 & 5.99 & 16.05 & 2.18 \\
 5 &  8.63 & 6.70 & 16.45 & 2.21 \\
 7 &  9.03 & 7.21 & 15.94 & 2.28 \\
10 & 10.17 & 7.43 & 16.44 & 2.40 \\
12 & 10.88 & 7.66 & 16.30 & 2.39 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Bench machine A (Intel i7-13700K, $N\!=\!5$ independent
reps on Berlin and NYC 500-OD subsets, $2{,}500$ obs per cell, 1{,}000
bootstrap resamples per CI). \emph{TEEG-ALT-v2 advantage over CSA
is statistically clean on both cities: CIs do not overlap with CSA's.}}
\label{tab:multirep-berlin-nyc}
\small
\begin{tabular}{llrrrr}
\toprule
City & Method & Median ms & CI95\% lo & CI95\% hi & Speedup vs CSA \\
\midrule
Berlin & HL              & 0.163  & 0.160  & 0.166  & $23.4\times$ \\
Berlin & \textbf{TEEG-ALT-v2}    & \textbf{0.862}  & \textbf{0.849}  & \textbf{0.874}  & $\mathbf{4.43\times}$ \\
Berlin & RAPTOR          & 1.856  & 1.803  & 1.930  & $2.06\times$ \\
Berlin & ULTRA           & 1.927  & 1.858  & 1.978  & $1.98\times$ \\
Berlin & CSA             & 3.821  & 3.711  & 3.914  & $1.00\times$ \\
\midrule
NYC    & HL              & 0.314  & 0.307  & 0.324  & $22.0\times$ \\
NYC    & \textbf{TEEG-ALT-v2}    & \textbf{1.381}  & \textbf{1.356}  & \textbf{1.401}  & $\mathbf{4.99\times}$ \\
NYC    & RAPTOR          & 3.541  & 3.434  & 3.649  & $1.95\times$ \\
NYC    & ULTRA           & 3.647  & 3.525  & 3.765  & $1.89\times$ \\
NYC    & CSA             & 6.896  & 6.792  & 6.977  & $1.00\times$ \\
\bottomrule
\end{tabular}
\end{table}
