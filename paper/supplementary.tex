...
\paragraph{Independent oracle: multi-pass CSA.}
To independently validate that the 17 residuals are real single-pass-CSA
blind spots and not TEEG bugs, we implemented Dibbelt et al.\
...
boarding flag each pass. Walks are $F_{15}$-transitively-closed
with a 15-min per-leg cap.

\textbf{Results on the 17 residuals.} Multi-pass CSA Algorithm~2
strictly improves over single-pass Algorithm~1 on \textbf{all 17}
queries (median $\Delta\!=\!-2$\,min, max $\Delta\!=\!-21$\,min).
Multi-pass CSA matches TEEG-ALT \textbf{bit-for-bit on 16/17} queries.
...
(Dibbelt et al.\ \cite{dibbelt2018csa}, Sec.~3.4), closed by the
multi-pass fixpoint variant. Per-query results in
\texttt{multipass\_csa\_oracle/results\_17.csv}; implementation in
...
information-set inequality, not a scoring error. The three candidate
scan-side root causes (service-day filter, int16-minutes encoding,
MCC stop set) remain the prime suspects.
...
The supplementary code release (linked from the de-anonymised version
at submission acceptance) contains the artefacts needed to regenerate
...
\paragraph{Cross-machine summary.}
\begin{itemize}
\item \emph{Berlin / NYC TEEG-ALT-v2 dominance over CSA}: machine
  A ratio $0.42$ (B/A $0.44$, $0.46$); robust within $\pm 5\%$.
\item \emph{Cycle 10 dynamic-update speedup}: $\geq 45{,}000\times$
  (machine A) and $\geq 10^6\times$ (machine B); robust.
\item \emph{Full London TEEG-ALT-v2 vs CSA}: machine A $0.92\times$
  (TEEG faster); machine B $1.55\times$ across 3 reps
  ($\sigma\!=\!0.05$, CSA faster). The two machines disagree; we
  treat this comparison as within hardware-noise floor on machine A
  and report machine B's quieter measurement as evidence of a CSA
  win on a less-contended host.
\end{itemize}
