...
processes single-edit GTFS-Realtime updates in $0.47\,\mathrm{ms}$
median wall-clock on Full London (machine B, Xeon 8260L):
$35.8\times$ faster than the realistic \emph{lazy CSA-resort}
baseline (apply edits in place, re-sort the connection array,
re-query; median over 12 cells, range $13.5\text{--}50\times$) and
$25{,}986\times$ faster than rebuilding the full TEEG from scratch
(the cost an HL-class preprocessing-heavy deployment pays per edit;
median, range $1{,}161\text{--}36{,}911\times$). Amortised per-edit
wall-clock in a batch of $N\!=\!1000$ edits drops to
$\sim\!30\,\mu\mathrm{s}$ (sub-millisecond for \textsc{cancel}).
...
TEEG-INCR is between $1{,}161\times$ and $36{,}911\times$ faster than
re-building the static index from scratch (median $25{,}986\times$ on
machine B, $45{,}755\times$ on machine A). Prior millisecond-scale
...
dominance is robust across machines (an $N\!=\!5$ multi-rep
bootstrap on 500-OD subset shows even larger TEEG-ALT-v2 ratios:
$4.43\times$ Berlin, $4.99\times$ NYC, non-overlapping CI95\%; supp
...
single-run, $4.43\times$ 5-rep bootstrap with non-overlapping CI95\%)
and NYC ($2.30\times$ single-run, $4.99\times$ 5-rep bootstrap; supp
...
$13.5\times\text{--}50.0\times$ faster than lazy CSA-resort
(median $35.8\times$ on machine B; $15.3\times$ on machine A).
Both ratios are honest: lazy CSA-resort's
...
\begin{table}[t]
\centering
\caption{TEEG-INCR vs lazy CSA-resort on Full London (300
OD/cell). Machine B (server pkg v3, 10-min walk-cap).}
\label{tab:lazy-csa}
\small
\begin{tabular}{rrrrr}
\toprule
$N$ & delay (×) & cancel (×) & add (×) & median \\
\midrule
1    & 39.6 & 50.0 & 43.2 & 43.2 \\
10   & 34.8 & 45.6 & 39.3 & 39.3 \\
100  & 29.9 & 36.8 & 28.6 & 29.9 \\
1000 & 26.2 & 13.5 & 23.9 & 23.9 \\
\bottomrule
\end{tabular}
\end{table}
...
We captured 24h of TfL line-status (5 modes; 1{,}428 snapshots
over 695 lines) + DfT BODS SIRI-VM (1{,}399 frames, 1.6M
vehicle-snapshots; 2026-06-13). Our static bundle is 2025-01-03
($\sim\!17$ months stale), so direct trip-id replay is infeasible.
We use \emph{statistical-pattern transfer}: captured distribution
drives a 1{,}000-edit synthetic stream against our 2025 bundle,
preserving operation mix (74\% delay / 20\% cancel / 6\% add),
rush-hour weighting ($\sim\!9\times$ peak), and delay magnitudes
(mean 4.6, median 3, p95 12 min). Our Cycle 10 synthetic-uniform
distribution at $N\!=\!1000$ per batch is \emph{strictly more
aggressive} than this real distribution (real peaks
$\sim\!99$ edits/h $\ll\!1000$/batch), so Table~\ref{tab:lazy-csa}'s
$35.8\times$ lazy-CSA-resort speedup is a \emph{lower bound} on
real-world performance. Scripts + capture in supp Appendix~E.
...
For deployment we recommend a 10-minute walk-cap. A finer
server-pkg-v3 sweep at $\{3,5,7,10,12\}$ min on machine B confirms
graceful scaling; TEEG-ALT-v2 latency grows $\sim\!33\%$ across
the 4$\times$ walk-cap increase, CSA $\sim\!28\%$; the
TEEG-ALT-v2-vs-CSA ratio stays bounded at $1.35\text{--}1.42\times$
on machine B (supp Table~S7).
