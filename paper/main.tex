...
London, Berlin, NYC); aggregated across all stress cells $0.33\%$ of
queries fall behind the oracle, bounded by $|\Delta| \leq 15$
minutes.} To our knowledge this is the first published
...
the same substrate. On Berlin and NYC, TEEG-ALT beats compiled CSA by
$2.3\times$ at $\geq\!99.3\%$ EA parity, reproducing across two
...
(ii) as an empirical observation (Proposition~\ref{thm:subsum},
...
reports seconds-scale batch update phases. None of these targets
sub-15\,ms per-edit incremental updates for transit EA at city scale
with formal correctness validation against a rebuild oracle. Our
TEEG-INCR fills this gap.
...
\begin{proposition}[Empirical subsumption]\label{thm:subsum}
For every Full-London query in our 1{,}500-query
...
We characterise the configurations under which this happens in
Appendix~A. We use the term \emph{empirical} subsumption because the
fully formal claim (TEEG's reachability set is a structural superset of
single-pass CSA's) requires a careful precondition on identical
walk-graph preprocessing across the two implementations, which we
discuss in Appendix~A and observe to hold for our Full-London setup.
...
noise floor and reverses on machine B (supp Table~S5); Berlin and
NYC dominance is robust across both machines.} NYC RAPTOR / ULTRA
parity $98.7\%\!=\!2962/3000$ is the structural reachability
coverage of the routes-array layout (Appendix~E); TEEG-ALT-v2 parity
$99.6\%\!=\!2950/2962$ is the algorithmic agreement on the reachable
subset. MG-Dial-v2 Full London uses the $K\!=\!\infty$ fast path
(post-fix; Appendix~F).}
\label{tab:per-city}
\small
...
Table~\ref{tab:per-city} reports the headline cross-city results on
bench machine A. TEEG-ALT-v2 dominates compiled CSA on Berlin
($2.34\times$) and NYC ($2.30\times$); on Full London the static
margin is small ($1.10\times$ on machine A) and reverses on
machine B (CSA $1.55\times$ faster across 3 reps, $\sigma=0.05$ ---
supp Table~S5), so we treat the London static-latency claim as
within-hardware noise and lead on Berlin/NYC for the dominance
story. TEEG-ALT-v2 preserves $\geq 99.3\%$ parity with the CSA
oracle on all three cities; the remaining $<\!1\%$ disagreements
are strict supersets in favour of TEEG-ALT-v2 (Section~6.3,
Proposition~1). HL remains $4\text{--}7\times$ faster than TEEG-ALT-v2
on static queries but at the cost of hours of preprocessing that
must be redone after every schedule edit (Section~6.4); HL also
...
\caption{Histogram of EA $\Delta$ on the queries where TEEG-ALT-v1 and
single-pass CSA disagree. \emph{Positive} $\Delta$ means TEEG returns a
strictly earlier arrival than CSA (the subsumption-strict case).
\emph{Negative} $\Delta$ in Berlin/NYC is attributable to walk-graph
preprocessing differences and is independently audited
(supplementary~D).}
\label{fig:subsumption}
\end{figure}

On Full London with 1{,}500 random queries, TEEG-ALT and single-pass CSA
disagree on 17 (1.1\%); see Figure~\ref{fig:subsumption}. We
independently validate \emph{all 17} two ways: (i) edge-by-edge
GTFS feasibility (every RIDE edge against \texttt{b.connections},
every WALK edge against $W/v_\text{walk}$, every TRANSFER edge
against the minimum-transfer-time; end-to-end simulation arrival
matches \texttt{ea\_teeg}); and (ii) multi-pass CSA Algorithm~2
\cite{dibbelt2018csa} as an independent oracle. \textbf{Multi-pass
CSA matches TEEG-ALT bit-for-bit on 16/17 residuals} and strictly
improves over single-pass CSA on all 17 (median
$\Delta\!=\!-2$\,min, max $\Delta\!=\!-21$\,min), confirming the
gap is the documented single-pass blind spot, not a TEEG bug. On
\texttt{q\_id=1497} multi-pass CSA finds an EA one minute earlier
than TEEG-ALT (separate ALT-pruning blind spot, footnote in
Appendix~A).

The 17 disagreements decompose into two failure modes:
\begin{enumerate}
\item \textbf{Source-walk suppresses transit walk-relax} (4/17,
$|\Delta| = 4\text{--}21$\,min, $n_\text{csa\_missed} > 0$):
$s_\text{src}$ has a fast same-stop bus, plus a slightly slower walk-to-tube
that opens a strictly faster Tube connection. CSA's source-init pass commits
to the same-stop bus; TEEG-ALT keeps both as competing events.
\item \textbf{Walk-from-walk doesn't chain} (13/17, $|\Delta| = 1\text{--}6$\,min,
$n_\text{csa\_missed} = 0$): a foot-segment ending
at stop $a$ enables a second foot-segment $a \to b$ that catches an earlier
Tube. CSA does not chain walks at relax time; TEEG-ALT does.
\end{enumerate}

We note (honesty disclosure) that this analysis was conducted with the help
of an LLM-assisted sub-agent reviewing trace-level path traces. We provide
the 17 raw path traces in the supplementary material so reviewers can verify
independently.
