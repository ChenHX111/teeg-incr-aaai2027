...
machine B with bootstrap 95\% CIs that are mutually exclusive
(CSA $8.34$\,ms [$7.82, 8.69$] vs TEEG-ALT-v2 $12.69$\,ms
[$11.57, 13.80$], $N\!=\!3$ reps $\times 498$ ODs $=\! 1494$
observations per cell, 1{,}000 bootstrap resamples; supp
Table~S5 and
\texttt{Experiments/results/cycle\_9\_paper/phase\_d\_multirep/}\allowbreak\texttt{bootstrap\_cis.csv}).
We therefore treat the London static-latency claim as a
within-hardware-class artefact, not a universal property, and lead
on Berlin/NYC for the dominance story. TEEG-ALT-v2 preserves
$\geq 99.3\%$ parity with the CSA oracle on all three cities; the
remaining $<\!1\%$ disagreements are strict supersets in favour of
TEEG-ALT-v2 (Section~6.3, Proposition~1). HL remains
$4\text{--}7\times$ faster than TEEG-ALT-v2 on static queries but at
the cost of hours of preprocessing that must be redone after every
schedule edit (Section~6.4); HL also does not support incremental
updates.
...
\paragraph{Real-trace validation (Phase E, planned).}
The Cycle~10 edit stream uses synthetic random \textsc{cancel}/
\textsc{delay}/\textsc{add} primitives drawn from each city's GTFS
trip set, which is a worst-case-uniform distribution. To validate
under the actual GTFS-RT distribution, we plan a 24-hour SIRI-VM +
SIRI-SX capture from the TfL Unified API and the DfT Bus Open Data
Service, then replay the captured edit stream through the same
incremental update path. The capture harness
(\texttt{Experiments/results/cycle\_9\_paper/phase\_e\_gtfs\_rt/}\allowbreak\texttt{phase\_e\_capture.py})
and the replay scaffold (\texttt{phase\_e\_replay.py}) are in the
anonymous code repo, pending API-key registration; this validation
is in the AAAI version's revision plan rather than the
