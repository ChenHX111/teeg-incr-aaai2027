...
def raptor_earliest_arrival(b: TimetableBundle, source: int, dest: int, t_dep: int,
                            max_rounds: int = 8,
                            closed_walks_csr: tuple | None = None) -> int | None:
    """RAPTOR earliest arrival.

    Parameters
    ----------
    closed_walks_csr : optional (indptr, indices, wmins) tuple in CSR layout
        for *transitively closed* walking footpaths. When provided, the
        transfer relaxation step uses this instead of the raw single-hop
        `transfer_array`. Required for fair comparison against CSA when the
        raw footpath set is not closed (see CSA notes).
    """
    ...
    if closed_walks_csr is not None:
        ...
        cw_indptr = cw_indices = cw_wmins = None
    ...
    marked[source]   = True
    if cw_indptr is not None:
        lo, hi = int(cw_indptr[source]), int(cw_indptr[source + 1])
        for k in range(lo, hi):
            v = int(cw_indices[k]); wmin = int(cw_wmins[k])
            nt = t_dep + wmin
            if nt < tau_best[v]:
                tau_best[v] = nt
                tau_prev[v] = nt
    ...
    for round_no in range(1, max_rounds + 1):
        ...
        # CORRECTNESS NOTE: we must read tau_best at marked stops via a SNAPSHOT,
        # not via the live array, otherwise a walk added earlier in this step
        # can be the source of a *further* walk in the same step, effectively
        # chaining two walks within a single round and exceeding the closure cap.
        # See loss diagnostic q=315 (RAPTOR returned 262 by chaining 15+12 = 27min walks).
        marked_now = np.where(marked)[0]
        tau_snapshot = tau_best[marked_now].copy()
        for idx_m, s in enumerate(marked_now):
            tau_s = int(tau_snapshot[idx_m])
            if cw_indptr is not None:
